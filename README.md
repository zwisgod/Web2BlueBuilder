...
如果你电脑已经安装 `Node.js 18+`、`Android SDK`、`JDK 17`，可以直接看“已有环境”。

如果你不想手动安装 Android Studio / Android SDK，可以先看“便携环境一键安装”。
...
- `node/`：便携 Node.js
- `jdk17/`：便携 JDK 17
- `android-sdk/`：便携 Android SDK 命令行环境

```powershell
git clone https://github.com/zwisgod/Web2BlueBuilder.git
cd Web2BlueBuilder
powershell -ExecutionPolicy Bypass -File .\setup-portable.ps1
.\web2blue.bat init demo-web
.\web2blue.bat build --src .\demo-web --name "Web2Blue演示"
```

构建成功后，APK 会出现在 `output/` 文件夹中。

说明：

- 第一次运行需要联网下载 Node.js、JDK 17、Android SDK 组件和 Gradle 依赖。
- 下载内容比较大，耗时取决于网络。
- 这个方案不需要安装完整 Android Studio。

## 已有环境

如果你已经有 Node.js、JDK 17 和 Android SDK，并且有一个网页项目，例如 `C:\my-web-app`，直接复制下面三行：

```powershell
git clone https://github.com/zwisgod/Web2BlueBuilder.git
cd Web2BlueBuilder
.\web2blue.bat build --src C:\my-web-app --name "我的控制器"
```

第一次运行时，`web2blue.bat` 会自动安装 CLI 依赖，不需要手动进入 `packages\cli`。

如果你只是想先体验一下，不知道 `--src` 应该填什么，可以直接跑示例：
...
构建成功后，APK 会出现在项目根目录的 `output/` 文件夹中。
...
```powershell
.\web2blue.bat build --src 你的网页目录 --name "应用名称"
```
...
- Node.js 18 或更高版本
- Android SDK，建议包含 API 34、Build Tools、Platform Tools
- JDK 17
...
脚本会自动下载并安装 Android SDK 命令行环境到项目根目录的 `android-sdk/`。

### 方式 B：安装 Android Studio

也可以安装 Android Studio：

1. 打开官方页面：https://developer.android.com/studio
2. 下载并安装 Windows 版 Android Studio。
3. 打开 Android Studio，进入 `More Actions -> SDK Manager`。
4. 在 `SDK Platforms` 勾选 `Android 14.0 / API 34`。
5. 在 `SDK Tools` 勾选：
...
Windows 默认 SDK 路径通常是：

```text
C:\Users\你的用户名\AppData\Local\Android\Sdk
```

Web2BlueBuilder 会自动尝试识别这个默认路径。

## 自己打离线包

如果你不想让每个用户都联网下载，可以先在自己电脑运行：
...
Web2BlueBuilder/
├── node/            # 便携 Node.js
├── android-sdk/     # 你提前下载好的 Android SDK
├── jdk17/           # 便携 JDK 17
├── packages/
└── web2blue.bat
```

Web2BlueBuilder 会自动按以下顺序寻找 SDK：

1. `ANDROID_SDK_ROOT`
2. `ANDROID_HOME`
3. 项目根目录下的 `android-sdk/`
4. Android Studio 默认路径 `%LOCALAPPDATA%\Android\Sdk`

注意：`node/`、`android-sdk/` 和 `jdk17/` 体积很大，不建议提交到 GitHub。更适合你自己打一个离线压缩包发给队友或现场演示电脑。
...
```powershell
.\web2blue.bat init my-blue-app
```

然后构建：

```powershell
.\web2blue.bat build --src .\my-blue-app --name "My Blue App"
```
